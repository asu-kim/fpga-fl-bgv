...
int main() {
    ...
    data_t in_data[IN_CHANNELS * IN_ROWS * IN_COLS];
    data_t out_data[IN_CHANNELS * OUT_ROWS * OUT_COLS];
    data_t out_data_ref[IN_CHANNELS * OUT_ROWS * OUT_COLS];
    ...
    for(int ch=0; ch < IN_CHANNELS; ch++) {
        for(int pr=0; pr < OUT_ROWS; pr++) {
            for(int pc=0; pc < OUT_COLS; pc++) {
                ...
                data_t avg = (sum + (POOL_SIZE*POOL_SIZE / 2)) / (POOL_SIZE*POOL_SIZE);
                // Apply min/max bounding
                avg = hls::max(MIN_VAL, hls::min(MAX_VAL, avg));
                // out_data_ref[ch + IN_CHANNELS*(pr*OUT_COLS + pc)] = avg;
