...
int main() {
    ...
    // Test 2: Record errors between original and decrypted data (without affecting test pass/fail status)
    std::cout << "----------------------------------------------" << std::endl;
    ...
    for(int i = 0; i < num_conv1_weights_slices; i++) {
        for(int j = 0; j < POLYNOMIAL_DEGREE; j++) {
            ...
            if (idx < NUM_CONV1_WEIGHTS) {
                ...
                if(i == 0 && j < 5) {
                    std::cout << "Sample at index " << j << ": Original = " << original 
                            << ", Decrypted = " << decrypted 
    ...
    std::cout << "  Average Relative Error: " << avg_rel_error * 100.0f << "%" << std::endl;
    std::cout << "----------------------------------------------" << std::endl;
