...
int main(int argc, char **argv)
{
    ...
    // Create kernels
    auto avg_pool2_bwd_krnl = xrt::kernel(device, uuid, "avg_pool2_bwd");

    std::cout << "Allocate Buffer in Global Memory\n";

    // Allocate in and out for avg_pool2_bwd
    auto bo_in_data = xrt::bo(device, in_size_bytes, avg_pool2_bwd_krnl.group_id(0));
    auto bo_out_data = xrt::bo(device, out_size_bytes, avg_pool2_bwd_krnl.group_id(1));
    ...
    auto run = avg_pool2_bwd_krnl(bo_in_data, bo_out_data);
    ...
    pool_bwd_golden<2, 2, 16, 8, 8>(bo_in_data_map, bo_out_data_golden);
