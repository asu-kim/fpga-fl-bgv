...
void parameter_decryption(
    ...
    // Control interface
    #pragma HLS INTERFACE s_axilite port=sk     bundle=control
    #pragma HLS INTERFACE s_axilite port=ct0    bundle=control
    #pragma HLS INTERFACE s_axilite port=ct1    bundle=control
    #pragma HLS INTERFACE s_axilite port=scale  bundle=control
    #pragma HLS INTERFACE s_axilite port=zp     bundle=control
    #pragma HLS INTERFACE s_axilite port=pt     bundle=control
    #pragma HLS INTERFACE s_axilite port=return bundle=control
    data_t local_sk[POLYNOMIAL_DEGREE];
    for(int i = 0; i < POLYNOMIAL_DEGREE; i++) {
        #pragma HLS PIPELINE II=1
        local_sk[i] = sk[i];
    }
    ...
    print_data_t_array_hls(local_sk, POLYNOMIAL_DEGREE, "local_sk");
    print_data_t_array_hls(local_ct0, POLYNOMIAL_DEGREE, "local_ct0");
    ...
    for(int i = 0; i < POLYNOMIAL_DEGREE; i++) {
        #pragma HLS PIPELINE II=1
        local_pt[i] = (quantized_pt[i] - zp) * scale;
    }
