...
void polynomial_multiplication(data_t* in1, data_t* in2, data_t* out) {
    ...
    for (int i = 0; i < POLYNOMIAL_DEGREE; i++) {
        ...
        in1_tilde[i] = modulo_reduction(E_POWERS_LUT[i] * in1[i], q);
        in2_tilde[i] = modulo_reduction(E_POWERS_LUT[i] * in2[i], q);
    }
    for (int i = 0; i < POLYNOMIAL_DEGREE; i++) {
    ...
    for (int i = 0; i < POLYNOMIAL_DEGREE; i++) {
        ...
        transformed_out_tilde[i] = modulo_reduction(transformed_in1_tilde[i] * transformed_in2_tilde[i], q);
    }
    for (int i = 0; i < POLYNOMIAL_DEGREE; i++) {
    ...
    for (int i = 0; i < POLYNOMIAL_DEGREE; i++) {
        ...
        out[i] = modulo_reduction(E_INV_POWERS_LUT[i] * out_tilde[i], q);
    }
}