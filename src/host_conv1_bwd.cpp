...
int main(int argc, char **argv)
{
    ...
    // Allocate buffers
    auto bo_in_activation = xrt::bo(device, sizeof(float) * in_activation_size, conv1_bwd_krnl.group_id(0));
    auto bo_grads = xrt::bo(device, sizeof(float) * grads_size, conv1_bwd_krnl.group_id(1));
    auto bo_in_weight = xrt::bo(device, sizeof(float) * in_weight_size, conv1_bwd_krnl.group_id(2));
    auto bo_out_grads = xrt::bo(device, sizeof(float) * out_grads_size, conv1_bwd_krnl.group_id(3));
    auto bo_dW = xrt::bo(device, sizeof(float) * dW_size, conv1_bwd_krnl.group_id(4));
    auto bo_dB = xrt::bo(device, sizeof(float) * dB_size, conv1_bwd_krnl.group_id(5));
