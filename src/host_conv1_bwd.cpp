...
int main(int argc, char **argv)
{
    ...
    // Create kernels
    auto conv1_bwd_krnl = xrt::kernel(device, uuid, "conv1_bwd");

    std::cout << "Allocate Buffer in Global Memory\n";

    // Allocate buffers
    auto bo_in_activation = xrt::bo(device, sizeof(data_ap_fixed_t) * in_activation_size, conv1_bwd_krnl.group_id(0));
    auto bo_grads = xrt::bo(device, sizeof(data_ap_fixed_t) * grads_size, conv1_bwd_krnl.group_id(1));
    auto bo_in_weight = xrt::bo(device, sizeof(data_ap_fixed_t) * in_weight_size, conv1_bwd_krnl.group_id(2));
    auto bo_out_grads = xrt::bo(device, sizeof(data_ap_fixed_t) * out_grads_size, conv1_bwd_krnl.group_id(3));
    auto bo_dW = xrt::bo(device, sizeof(data_ap_fixed_t) * dW_size, conv1_bwd_krnl.group_id(4));
    auto bo_dB = xrt::bo(device, sizeof(data_ap_fixed_t) * dB_size, conv1_bwd_krnl.group_id(5));
    ...
    // Run Conv1 layer backward path
    std::cout << "Running Conv1 Backward\n";
    auto run = conv1_bwd_krnl(bo_in_activation, bo_grads, bo_in_weight, bo_out_grads, bo_dW, bo_dB);
