...
def generate_header(txt_files, header_path):
    ...
    lines_out.append("#endif // WEIGHTS_BIAS_H\n")
...
def main():
    patterns = [
        "quant_data/conv*_weight*.txt",
        "quant_data/conv*_bias*.txt",
        "quant_data/fc*_weight*.txt",
        "quant_data/fc*_bias*.txt",
    ]
    txts = []
    for p in patterns:
    ...
    if not txts:
        ...
        sys.exit(1)
    generate_header(txts, "data/weights_bias.h")
